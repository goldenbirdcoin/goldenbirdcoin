...
// Copyright (c) 2016 The GoldenBird developers
...
const string strMessageMagic = "GoldenBird Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // GoldenBird: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // GoldenBird: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    {
        nSubsidy = 445500000 * COIN;
    }
	    else if(pindexBest->nHeight+1 >= 2 && pindexBest->nHeight+1 <= 50)
    {
        nSubsidy = 0 * COIN;
    }
        else if(pindexBest->nHeight+1 >= 51 && pindexBest->nHeight+1 <= 210240)
    {
        nSubsidy = 5 * COIN;
    }
		else if(pindexBest->nHeight+1 >= 210241 && pindexBest->nHeight+1 <= 420480)
    {
        nSubsidy = 4 * COIN;
    }
	else if(pindexBest->nHeight+1 >= 420481 && pindexBest->nHeight+1 <= 630720)
    {
        nSubsidy = 3 * COIN;
    }
	else if(pindexBest->nHeight+1 >= 630720 && pindexBest->nHeight+1 <= 1000000)
    {
        nSubsidy = 2 * COIN;
    }
...
// GoldenBird: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes GoldenBird fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // GoldenBird: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // GoldenBird: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // GoldenBird: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // GoldenBird: track money supply and mint amount info
...
// GoldenBird: total coin age spent in transaction, in the unit of coin-days.
...
// GoldenBird: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // GoldenBird: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // GoldenBird: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // GoldenBird: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // GoldenBird: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // GoldenBird: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // GoldenBird: check proof-of-stake
    ...
    // GoldenBird: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // GoldenBird: getblocks may not obtain the ancestor block rejected
    ...
    // GoldenBird: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "GoldenBird", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "GoldenBird CryptoCurrency";
        CTransaction txNew;
        txNew.nTime = 1481095456;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1481095456;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 4762408 : 16355;

        if(true && (block.GetHash() != hashGenesisBlockTestNet))
		  {
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("5887be412b9ce1ece6903297925516adc937695b92301f40a84d74a35941340e"));
        block.print();
        ...
        // GoldenBird: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // GoldenBird: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // GoldenBird: tell downloading node about the latest block if it's
